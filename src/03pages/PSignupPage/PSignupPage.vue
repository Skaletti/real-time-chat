  ...
  <div class="authorization">
    <div class="container">
      ...
      <el-form
        ...
        @submit.prevent
      >
        <h4 class="mb-12">Signup to Chat</h4>
        <s-form-item class="authorization-form-item" prop="name">
          <s-input
        ...
        <div class="authorization-form__controls">
          ...
          <s-button appearance="link" :as="'router-link'" :to="ROUTE_NAMES.authorization"> Back to Login</s-button>
        </div>
      </el-form>
    </div>
  </div>
...
import { auth } from 'apps/services/firebase'
import { firebaseApp } from 'apps/services/firebase'
...
const submitForm = async () => {
  const auth = getAuth()
  const name = ruleForm.value.name
    ...
    if (valid) {
      try {
        ...
        if (user) {
          ...
          await router.push(ROUTE_NAMES.chat)
        }
      } catch (error) {
        console.log(error)
      }
